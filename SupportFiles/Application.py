...
class Application:
    ...
    def copy_to_property(self):
        Actions.find_Element(self.browser, "InsuredResidentAddr.Addr1").send_keys(self.address1)
        if self.address2 is not None:
            Actions.find_Element(self.browser, "InsuredResidentAddr.Addr1").send_keys(self.address2)
        Actions.find_Element(
            self.browser, "InsuredResidentAddr.City").send_keys(self.city)
        Select(Actions.find_Element(self.browser, "InsuredResidentAddr.StateProvCd")).select_by_value(self.state)
    ...
    def copy_to_mailing(self):
        Actions.find_Element(self.browser, "InsuredMailingAddr.Addr1").send_keys(self.address1)
        if self.address2 is not None:
            Actions.find_Element(self.browser, "InsuredResidentAddr.Addr1").send_keys(self.address2)
        Actions.find_Element(self.browser, "InsuredMailingAddr.City").send_keys(self.city)
        Select(Actions.find_Element(self.browser, "InsuredMailingAddr.StateProvCd")).select_by_value(self.state)
    ...
    def startApplication(self, multiAdd, subType, carrier):
        ...
        self.core_coverages = CoreCoverages(self.browser, self.dwelling_program)
        self.underwriting = Underwriting(self.browser, self.state_chosen, self.line_of_business, self.number_of_addresses)

        # *Tab to click  for recent quotes, applications, and policies
        Actions.find_Element(self.browser, "Tab_Recent").click()
        self.state, self.city, self.address1 = Address.addresses[str(state_chosen)]
        ...
        self.create_new_quote(date_chosen, producer_selected, first_name,last_name, multiAdd, self.TEST, subType, CARRIER[carrier])
    ...
    def submit_policy(self):
        ...
        Actions.waitPageLoad(self.browser)
        Select(Actions.find_Element(self.browser, "TransactionInfo.PaymentTypeCd")).select_by_value("None")
        Actions.find_Element(self.browser, "TransactionInfo.SignatureDocument").click()
    ...
    def create_new_quote(self, date, producer: str, first_name: str, last_name: str, multiLoc: bool, test: bool, subType: str, carrier: str):
        ...
        # New Quote
        Actions.find_Element(self.browser, "QuickAction_NewQuote_Holder").click()
        Actions.find_Element(self.browser, "QuickAction_EffectiveDt").send_keys(date)

        Actions.waitPageLoad(self.browser)
        # State Select
        self.browser.execute_script("document.getElementById('QuickAction_StateCd').value = '"+self.state_chosen+"';")
        Actions.check_for_value(self.browser, "QuickAction_CarrierGroupCd", self.COMPANY)

        self.browser.execute_script("document.getElementById('QuickAction_NewQuote').click()")
        Actions.waitPageLoad(self.browser)

        if self.line_of_business == "Personal Umbrella":
            Actions.find_Element(self.browser, "Homeowners", By.LINK_TEXT).click()
        elif self.line_of_business == "Commercial Umbrella":
            Actions.find_Element(self.browser, "Businessowners", By.LINK_TEXT).click()
        else:
        ...
        if (self.line_of_business == "Dwelling Property" or self.line_of_business == "Businessowners" or self.line_of_business == "Commercial Umbrella"):
            Select(Actions.find_Element(self.browser, "Insured.EntityTypeCd")).select_by_value("Individual")

        Actions.waitPageLoad(self.browser)

        quote_num = Actions.find_Element(self.browser, "QuoteAppSummary_QuoteAppNumber")
        ...
        Actions.check_for_value(self.browser, "InsuredPersonal.OccupationClassCd", "Other")
        Actions.check_for_value(self.browser, "InsuredPersonal.OccupationOtherDesc", keys="No")

        if self.state_chosen == "NY" and (self.line_of_business == "Homeowners" or self.line_of_business == "Personal Umbrella"):
            Select(Actions.find_Element(self.browser, "BasicPolicy.GeographicTerritory")).select_by_value("Upstate")

        self.browser.execute_script('document.getElementById("InsuredName.GivenName").value = "' + first_name + '"')
        if (self.mid_name is not None) and (self.mid_name != ""):
            self.browser.execute_script('document.getElementById("InsuredName.OtherGivenName").value = "' + self.mid_name + '"')
        self.browser.execute_script('document.getElementById("InsuredName.Surname").value = "' + last_name + '"')

        Actions.check_for_value(self.browser, "InsuredNameJoint.GivenName", keys="click")
        Actions.check_for_value(self.browser, "InsuredNameJoint.GivenName", keys="Second")
        Actions.check_for_value(self.browser, "InsuredNameJoint.Surname", keys="Person")
        Actions.waitPageLoad(self.browser)
        Actions.check_for_value(self.browser, "InsuredNameJoint.GivenName", keys="Second")
        Actions.check_for_value(self.browser, "InsuredNameJoint.Surname", keys="Person")
        Actions.check_for_value(self.browser, "InsuredPersonalJoint.BirthDt", keys="01/01/1980")
        Actions.check_for_value(self.browser, "InsuredPersonalJoint.OccupationClassCd", "Other")
        Actions.check_for_value(self.browser, "InsuredPersonalJoint.OccupationOtherJointDesc", keys="No")

        if (self.line_of_business == "Homeowners" or self.line_of_business == "Personal Umbrella") and subType:
            Actions.check_for_value(self.browser, "BasicPolicy.DisplaySubTypeCd", subType)
            if self.state_chosen == "NY":
                Select(Actions.find_Element(self.browser, "BasicPolicy.DisplaySubTypeCd")).select_by_index(1)

        if self.line_of_business != "Businessowners" and self.line_of_business != "Commercial Umbrella":
            Actions.find_Element(self.browser, "InsuredPersonal.BirthDt").send_keys("01/01/1980")
            Actions.find_Element(self.browser, "InsuredCurrentAddr.Addr1").send_keys(self.address1)
            if self.address2 is not None:
                Actions.find_Element(self.browser, "InsuredCurrentAddr.Addr2").send_keys(self.address2)
            Actions.find_Element(self.browser, "InsuredCurrentAddr.City").send_keys(self.city)
            Select(Actions.find_Element(self.browser, "InsuredCurrentAddr.StateProvCd")).select_by_value(self.state)
        ...
        if (self.line_of_business == "Businessowners" or self.line_of_business == "Commercial Umbrella"):
            Actions.find_Element(self.browser, "InsuredMailingAddr.Addr1").send_keys(self.address1)
            if self.address2 is not None:
                Actions.find_Element(self.browser, "InsuredMailingAddr.Addr1").send_keys(self.address2)
            Actions.find_Element(self.browser, "InsuredMailingAddr.City").send_keys(self.city)
            Select(Actions.find_Element(self.browser, "InsuredMailingAddr.StateProvCd")).select_by_value(self.state)

        # *Adding geographic territory and policy carrier here
        if (self.state_chosen == "NY" and (self.line_of_business == "Homeowners" or self.line_of_business == "Personal Umbrella")):
            Select(Actions.find_Element(self.browser, "BasicPolicy.GeographicTerritory")).select_by_value("Metro")
        ...
        Actions.check_for_value(self.browser, "Insured.InspectionContact", keys="None")
        Actions.check_for_value(self.browser, "InsuredPhonePrimary.PhoneName", "Mobile")
        Actions.find_Element(self.browser, "InsuredPhonePrimary.PhoneNumber").send_keys(5558675309)
        Actions.check_for_value(self.browser, "Insured.InspectionContactPhoneType", "Mobile")
        Actions.check_for_value(self.browser, "Insured.InspectionContactNumber", keys=5558675309)
        Actions.find_Element(self.browser, "InsuredEmail.EmailAddr").send_keys("test@mail.com")
        Actions.waitPageLoad(self.browser)

        # Set insurance score if available
        Actions.check_for_value(self.browser, "InsuredInsuranceScore.OverriddenInsuranceScore", keys="999")
        ...
        Actions.check_for_value(self.browser, "BasicPolicy.PolicyCarrierCd", carrier)
        ...
        #if multiLoc and self.line_of_business == "Businessowners":
        #    for i in range(self.number_of_addresses-1):
        #        Actions.find_Element(self.browser, "CopyRisk").click()
        #        Actions.save(self.browser)
        ...
        if (self.create_type == "Application" or self.create_type == "Policy"):
            ...
            if (self.state_chosen == "NJ" and (self.line_of_business == "Homeowners" or self.line_of_business == "Personal Umbrella")):
                ...
                Actions.waitPageLoad(self.browser)
                Actions.check_for_value(self.browser, "Building.Inspecti onSurveyReqInd", "No")
