...
class Interface:
    ...
    def make_window(self):
        ...
        all_tabs_info = [
            [sg.Text('Select Local or QA Environment'), sg.DropDown(list(self.gw_environment.keys()),enable_events=True,key="-ENVLIST-")],
            [sg.Text('Select Browser'), sg.DropDown(browsers, key = "BROWSER")],
            [sg.Text("Select Producer"),sg.DropDown(list(File.env_files_plus_users[self.env_used]["Producers"]["ProducerNames"]),size=(self.TEXTLEN,1),key="-PRODUCER-"),sg.Push(),sg.Button("Delete",size=(10,1),key="-REMPROD-")],
        ...
        new_app_layout = [  
            ...
            [sg.Text("                      Address 2",visible=False,justification="left", key = "-AddText2-"),sg.InputText(size = (self.TEXTLEN,1),visible=False, key = "-CADD2-")],
            [sg.Text("City (Required)",visible=False,justification="left", key = "-CityText-"),sg.Text("            "),sg.InputText(size = (self.TEXTLEN,1),visible=False, key = "-CITY-")],
            ...
            sg.DropDown(list(self.payment_plan_bop.keys()),visible=False,default_value="Direct Bill Full Pay",enable_events=True,key="-PAYPLANBOP-"),
        ...
        new_user_layout = [
            ...
            [sg.Text()],
            [sg.Text('Add User')],
            [sg.Text("Username"),sg.InputText(do_not_clear=False,size=(self.TEXTLEN,1),key="USER")],
            [sg.Text("Password"),sg.InputText(do_not_clear=False,size=(self.TEXTLEN,1),key="PASS")],
            [sg.Button("Add",key="-ADDU-")],
            [sg.Text()],
            [sg.Text("Create User in Andover (Local Only)",key="-CREATE_TEXT-",enable_events=True,visible=False)],
            [sg.DropDown(list(self.user_dict.keys()),key="UserDrop",enable_events=True,visible=False)],
            [sg.Button("Create",key="-CREATE_USER-",enable_events=True,visible=False)]
        ]
        
        exist_app_layout = [
            [sg.Text('Enter Information for An Existing Application')],
            [sg.Text("Application Number"), sg.InputText(size=(self.TEXTLEN,1))]
        ]
        ...
        # Create the Window
        window = sg.Window(f"Automation for Andover                                       Version: {self.VERSION}", layout)
        ...
        while True:
            ...
            if event == "-REMPROD-" and len(File.env_files_plus_users[self.env_used]["Producers"]["ProducerNames"]) > 0 and producer!="":
                File.env_files_plus_users[self.env_used]["Producers"]["ProducerNames"].remove(producer)
                prod_list = File.env_files_plus_users[self.env_used]["Producers"]["ProducerNames"]
                File.write_producer(File.folder+File.env_files_plus_users[self.env_used]["Producers"]["file"],prod_list)
